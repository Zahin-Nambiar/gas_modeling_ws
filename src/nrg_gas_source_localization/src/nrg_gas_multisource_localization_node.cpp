...
typedef message_filters::sync_policies::ApproximateTime<GasConcentration, AnemometerMsg, Odometry> MySyncPolicy;
...
{
    Odometry non_constptr_odom;
    non_constptr_odom.twist.twist.linear.x = odom->twist.twist.linear.x;
    non_constptr_odom.twist.twist.linear.y = odom->twist.twist.linear.y;
    non_constptr_odom.twist.twist.angular.z = odom->twist.twist.angular.z;
    ...
    multisource_localization_node->update( non_constptr_gas, non_constptr_wind, non_constptr_odom);
} 
...
{   
    ...
    sub_odom.subscribe(nh, "/odometry/filtered_map", 20);
    ...
    sync.reset(new nrg_gas::Sync(nrg_gas::MySyncPolicy(100), sub_gas, sub_wind, sub_odom));   
    sync->registerCallback(boost::bind(&nrg_gas::measurementCallback, _1, _2, _3));
